...
class vec3      //Класс вектора
{
public:
    float x, y, z, w;
    vec3() : x(0), y(0), z(0), w(1) {}
    vec3(float xx, float yy, float zz) : x(xx), y(yy), z(zz), w(1) {}
    friend vec3 operator + (vec3 v1, vec3 v2) { return vec3(v1.x + v2.x, v1.y + v2.y, v1.z + v2.z); }   // Сумма векторов
    friend vec3 operator - (vec3 v1, vec3 v2) { return vec3(v1.x - v2.x, v1.y - v2.y, v1.z - v2.z); }   // Разница векторов
    friend vec3 operator * (vec3 v, float f) { return vec3(v.x * f, v.y * f, v.z * f); }    // Умножение вектора на число
    friend vec3 operator / (vec3 v, float f) { return vec3(v.x / f, v.y / f, v.z / f); }    // Деление вектора на число
    void operator += (vec3 v) { x += v.x; y += v.y; z += v.z; }     // Сумма векторов с присвоением
    void operator -= (vec3 v) { x -= v.x; y -= v.y; z -= v.z; }     // Разница векторов с присвоением
    void operator *= (float f) { x *= f; y *= f; z *= f; }     // Умножение вектора на число с присвоением
    void operator /= (float f) { x /= f, y /= f, z /= f; }     // Деление вектора на число с присвоением
    friend vec3 operator - (vec3 v) { return v * -1; }           // Смена знака вектора
    friend float dot_prod(vec3 v1, vec3 v2) { return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z; }     // Умножение вектора на вектор
